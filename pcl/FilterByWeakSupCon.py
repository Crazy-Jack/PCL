...
class SupConFilterByWeakLabels(nn.Module):
    ...
    @torch.no_grad()
    def filterPriority(self, hard_example_mask, cluster_mask, weak_mask, device, priority="supervised"):
        """construct mask based on mask priority, default is label > cluster mask"""
        if priority == "supervised":
            priority_mask = weak_mask
            normal_mask = cluster_mask
    ...
    def forward(self, features, cluster_labels, weak_labels, priority="supervised"):
        ...
        mask = self.filterPriority(hard_example_mask, cluster_mask, weak_mask, device, priority=priority)
        # print("Final mask")
        # print(mask)
        ...
        return loss, hard_example_mask.sum()
