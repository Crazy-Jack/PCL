"""Filter the weak supervision signals by overclustered signals"""
...
class SupConFilterByCluster(nn.Module):
    """Filter the weak supervision signals by overclustered signals"""
    def __init__(self, temperature=0.07, base_temperature=0.07, threshold=0.1):
        super(SupConFilterByCluster, self).__init__()
        self.temperature = temperature
        self.base_temperature = base_temperature
        self.threshold = threshold
        print("Using SupConFilterByCluster")
    ...
    @torch.no_grad()
    def correct_weaklabels(self, hard_example_mask, cluster_mask, weak_mask, device):
        """becasue the weaklabels is less than the total class num, it make sense to use the clustering results to correct those whose dot product is small but within the same weak clusters"""
        hard_weak_mask = weak_mask * hard_example_mask
        correct_by_clustering = hard_weak_mask * cluster_mask

        # non hard weak mask
        non_hard_weak_mask = (torch.FloatTensor([1.0]).to(device) - hard_weak_mask) * weak_mask

        return correct_by_clustering + non_hard_weak_mask
    ...
    def forward(self, features, cluster_labels, weak_labels, priority="supervised"):
        ...
        mask = self.correct_weaklabels(hard_example_mask, cluster_mask, weak_mask, device)
        # print("Final mask")
        # print(mask)
